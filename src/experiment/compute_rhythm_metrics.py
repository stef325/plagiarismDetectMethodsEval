"""Pipeline para cálculo das métricas de similaridade rítmica."""
...
class RhythmMetricSummary:
    """Resumo da execução das métricas rítmicas."""
...
def compute_rhythm_metrics(
    ...
    """Executa as métricas de similaridade sobre as transformações rítmicas."""

    transformations_root = Path(transformations_path)
    representations_root = Path(representations_path)
    output_root = Path(output_path) / "rhythm"
    rhythm_transformations_root = transformations_root / "rhythm"
    inspection_date = datetime.now()
    start_time = time.perf_counter()

    if not rhythm_transformations_root.is_dir():
        raise FileNotFoundError(
            f"Diretório de transformações rítmicas não encontrado: {rhythm_transformations_root}"
        )
    ...
    metadata_files = sorted(
        path
        for path in rhythm_transformations_root.rglob("metadata.csv")
        if "validation" not in path.parts
    )
    if not metadata_files:
        raise ValueError("Não há transformações rítmicas para calcular métricas.")

    output_root.mkdir(parents=True, exist_ok=True)
    metrics_csv_path = output_root / "rhythm_similarity_metrics.csv"

    existing_rows = _load_existing_rows(metrics_csv_path)
    cached_keys = {row_key(row) for row in existing_rows}

    ngram_metric = RhythmNGramSimilarityMetric()
    ioi_metric = IoISimilarityMetric()
    edit_metric = RhythmicEditDistanceMetric()
    ...
    print("Iniciando o cálculo das métricas rítmicas...")

    for metadata_path in metadata_files:
        metadata_rows = _load_metadata_rows(metadata_path)
        for row in metadata_rows:
            original = RhythmRepresentation.from_dict(
            ...
            comparison_pairs = [
                ...
                (
                    ...
                    "original_copy",
                    original,
                    RhythmRepresentation.from_dict(original.to_dict()),
                ),
            ]
            metric_specs = [
                ("rhythm_ngram_similarity", {"n": rhythm_ngram_n}),
                ("ioi_similarity", {}),
...
def _compute_metric(
    ...
    if metric_name == "rhythm_ngram_similarity":
        return ngram_metric.compute(original, transformed, **metric_parameters)
    if metric_name == "ioi_similarity":
        return ioi_metric.compute(original, transformed)
    if metric_name == "rhythmic_edit_distance":
        return edit_metric.compute(original, transformed)
    raise ValueError(f"Métrica rítmica não suportada: {metric_name}")
...
def _print_summary(summary: RhythmMetricSummary, metrics_csv_path: Path) -> None:
    """Exibe um resumo amigável da execução."""

    print("Cálculo das métricas rítmicas concluído.")
