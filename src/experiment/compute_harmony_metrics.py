"""Pipeline para cálculo das métricas de similaridade harmônica."""
...
class HarmonyMetricSummary:
    """Resumo da execução das métricas harmônicas."""
...
def compute_harmony_metrics(
    ...
    """Executa as métricas de similaridade sobre as transformações harmônicas."""

    transformations_root = Path(transformations_path)
    representations_root = Path(representations_path)
    output_root = Path(output_path) / "harmony"
    harmony_transformations_root = transformations_root / "harmony"
    inspection_date = datetime.now()
    start_time = time.perf_counter()

    if not harmony_transformations_root.is_dir():
        raise FileNotFoundError(
            f"Diretório de transformações harmônicas não encontrado: {harmony_transformations_root}"
        )
    ...
    metadata_files = sorted(
        path
        for path in harmony_transformations_root.rglob("metadata.csv")
        if "validation" not in path.parts
    )
    if not metadata_files:
        raise ValueError("Não há transformações harmônicas para calcular métricas.")

    output_root.mkdir(parents=True, exist_ok=True)
    metrics_csv_path = output_root / "harmony_similarity_metrics.csv"

    existing_rows = _load_existing_rows(metrics_csv_path)
    cached_keys = {row_key(row) for row in existing_rows}

    chord_ngram_metric = ChordNGramSimilarityMetric()
    edit_metric = HarmonicEditDistanceMetric()
    pitch_class_metric = PitchClassSimilarityMetric()
    ...
    print("Iniciando o cálculo das métricas harmônicas...")

    for metadata_path in metadata_files:
        metadata_rows = _load_metadata_rows(metadata_path)
        for row in metadata_rows:
            original = HarmonyRepresentation.from_dict(
            ...
            comparison_pairs = [
                ...
                (
                    ...
                    "original_copy",
                    original,
                    HarmonyRepresentation.from_dict(original.to_dict()),
                ),
            ]
            metric_specs = [
                ("chord_ngram_similarity", {"n": chord_ngram_n}),
            ...
            for comparison_type, transformation_label, source_representation, target_representation in comparison_pairs:
                ...
                for metric_name, metric_parameters in metric_specs:
                    ...
                    value = _compute_metric(
                        ...
                        chord_ngram_metric=chord_ngram_metric,
                        edit_metric=edit_metric,
                        pitch_class_metric=pitch_class_metric,
                    )
                    new_rows.append(
                        {
                            "song_id": row["song_id"],
...
def _compute_metric(
    ...
    if metric_name == "chord_ngram_similarity":
        return chord_ngram_metric.compute(original, transformed, **metric_parameters)
    if metric_name == "harmonic_edit_distance":
        return edit_metric.compute(original, transformed)
    if metric_name == "pitch_class_similarity":
        return pitch_class_metric.compute(original, transformed)
    raise ValueError(f"Métrica harmônica não suportada: {metric_name}")
...
def _print_summary(summary: HarmonyMetricSummary, metrics_csv_path: Path) -> None:
    """Exibe um resumo amigável da execução."""

    print("Cálculo das métricas harmônicas concluído.")
